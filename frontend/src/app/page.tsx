...
import {
  ...
  Play, RefreshCw, AlertTriangle, Lock, CheckCircle2, XCircle, ArrowRight
} from "lucide-react";
import {
  governAction, executeMockCatalog, executeMockPayment, getPendingReviews,
  submitHumanReview, getAuditEvents, verifyAuditIntegrity, triggerTamperTest,
...
export default function ControlCenter() {
  ...
  const runScenarioA = async () => {
    ...
    try {
      const actionPayload = {
        ...
        parameters: { item_id: "cat_faq_01", query: "Fetch vendor onboarding payment FAQ terms" },
  ...
  // SCENARIO B: HIGH IMPACT PAYMENT (HUMAN REJECT -> CALLS = 0)
  const runScenarioB = async () => {
    ...
    try {
      ...
      // Attempt direct execution without token to prove Target Calls = 0
  ...
  // SCENARIO C: MALICIOUS INJECTION (BLOCK -> CALLS = 0)
  const runScenarioC = async () => {
    ...
    try {
      ...
      // Direct target attempt without token -> Must fail
  ...
  const runApprovedPaymentPath = async () => {
    setIsLoading(true);
    setTargetExecutionResult(null);
    try {
      ...
      // Automatically approve human review for demo flow
      ...
      <div className="sidebar">
        <div className="logo-container">
          ...
          <div>
            ...
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>V2 INTERVENTION LAYER</div>
          </div>
        </div>
        ...
        <div className="nav-links">
          ...
          >
            ...
            {pendingReviews.length > 0 && (
              <span className="badge badge-review" style={{ marginLeft: 'auto' }}>
      ...
      <div className="main-content">
        <div className="header-bar">
          <div>
            <div className="page-title">
              ...
              {activeTab === 'hero' && 'Live AI Agent Action Interceptor (Hero Demo)'}
              {activeTab === 'risk' && 'Multi-Dimensional Risk Inspection Engine'}
            ...
            <div className="page-subtitle">
              ...
              {activeTab === 'hero' && 'Demonstrates deterministic action authorization, dynamic L0-L3 assurance routing, and target call enforcement.'}
              {activeTab === 'risk' && 'Deep-dive inspection into Performance, Cost, Responsibility, and Business Impact formulas.'}
        ...
        {activeTab === 'command' && (
          <div>
            <div className="kpi-grid">
              ...
              <div className="glass-card kpi-card">
                ...
                <span className="kpi-value" style={{ color: 'var(--accent-amber)' }}>
                  {metrics?.pending_human_reviews || 0}
                </span>
              </div>
              <div className="glass-card kpi-card">
            ...
            {/* Target Execution Counter Box */}
            <div className="glass-card" style={{ marginBottom: '2rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                <div>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 600 }}>Mock Target System Execution Counters</h3>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Proves that rejected or un-authorized actions result in 0 target API calls.</p>
                </div>
            ...
            <div className="glass-card">
              <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem' }}>Recent Governance Interceptions</h3>
              {auditData?.events?.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>No governance events logged yet. Trigger a hero scenario above!</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {auditData?.events?.slice(-5).reverse().map((evt: any) => (
                    <div key={evt.event_id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                      <div>
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          <span className="badge badge-l0">{evt.assurance_level}</span>
                          <span className={`badge ${evt.decision === 'ALLOW' ? 'badge-allow' : evt.decision === 'HUMAN_REVIEW' ? 'badge-review' : 'badge-block'}`}>
                            {evt.decision}
                          </span>
                          <span style={{ fontWeight: 600, fontSize: '0.9rem' }}>{evt.action_type}</span>
                        </div>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.25rem' }}>
                          Event: {evt.event_type} | Policy: {evt.policy_id} v{evt.policy_version} | App: {evt.application_id}
                        </div>
                      </div>
        ...
        {/* ---------------------------------------------------- */}
        {/* TAB 2: LIVE HERO INTERCEPTOR */}
        {/* ---------------------------------------------------- */}
        {activeTab === 'hero' && (
          <div className="hero-grid">
            {/* Left Column: Interactive Scenario Trigger Controls */}
            <div className="glass-card">
              <h3 style={{ fontSize: '1.2rem', fontWeight: 600, marginBottom: '0.5rem' }}>Hero Scenario Triggers</h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1.25rem' }}>
                Select a scenario to trigger the full ControlPlane governance pipeline and verify target execution behavior.
              </p>

              <div className="scenario-btn-group">
                <button className="btn btn-scenario-safe" onClick={runScenarioA} disabled={isLoading}>
                  <div>
                    <div style={{ fontWeight: 700 }}>SCENARIO A — SAFE QUERY</div>
                    <div style={{ fontSize: '0.75rem', opacity: 0.8 }}>Customer FAQ Request → L0 → ALLOW → Target Calls = 1</div>
                  </div>
                  <ArrowRight size={16} />
                </button>
                ...
                <button className="btn btn-scenario-impact" onClick={runScenarioB} disabled={isLoading}>
                  <div>
                    <div style={{ fontWeight: 700 }}>SCENARIO B — HIGH IMPACT PAYMENT (REJECT)</div>
                    <div style={{ fontSize: '0.75rem', opacity: 0.8 }}>₹50L Payment → L3 → HUMAN_REVIEW → Reject → Target Calls = 0</div>
                  </div>
                  <ArrowRight size={16} />
                </button>
                ...
                <button className="btn btn-scenario-malicious" onClick={runScenarioC} disabled={isLoading}>
                  <div>
                    <div style={{ fontWeight: 700 }}>SCENARIO C — MALICIOUS INJECTION</div>
                    <div style={{ fontSize: '0.75rem', opacity: 0.8 }}>Prompt Injection / Secret Leak → BLOCK → Target Calls = 0</div>
                  </div>
                  <ArrowRight size={16} />
                </button>

                <button className="btn btn-primary" onClick={runApprovedPaymentPath} disabled={isLoading}>
                  <div>
                    <div style={{ fontWeight: 700 }}>APPROVED PAYMENT PATH</div>
            ...
            {/* Right Column: Live Pipeline Evaluation State Display */}
            <div>
              <div className="glass-card" style={{ marginBottom: '1.5rem' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem' }}>Governance Pipeline Output</h3>
                    ...
                    <div className="code-box">
                      {lastGovernanceResult.approval_token ? (
                        ...
                        <span style={{ color: 'var(--accent-rose)' }}>
                          NO TOKEN ISSUED (Target Execution Blocked)
                        </span>
                      )}
                    </div>
                  </div>
              ...
              {/* Target System Verification Output Box */}
              <div className="glass-card">
                ...
                {!targetExecutionResult ? (
                  ...
                  <div>
                    ...
                    <div className="code-box">
                      {JSON.stringify(targetExecutionResult, null, 2)}
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* ---------------------------------------------------- */}
        {/* TAB 3: RISK ANALYSIS ENGINE */}
        {/* ---------------------------------------------------- */}
        {activeTab === 'risk' && (
          <div>
            <div className="glass-card" style={{ marginBottom: '1.5rem' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: 600, marginBottom: '0.5rem' }}>Multi-Dimensional Risk Model Specification</h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                ControlPlane calculates risk across 4 independent technical dimensions rather than relying on a single unexplained score.
              </p>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
                <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontWeight: 600, color: 'var(--accent-cyan)', marginBottom: '0.25rem' }}>Performance Risk (R_perf)</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Evaluates evidence grounding status (SUPPORTED, UNVERIFIED, CONTRADICTED) and claim similarity.</div>
                </div>
                <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontWeight: 600, color: 'var(--accent-amber)', marginBottom: '0.25rem' }}>Cost Risk (R_cost)</div>
                ...
                <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontWeight: 600, color: 'var(--accent-rose)', marginBottom: '0.25rem' }}>Responsibility Risk (R_resp)</div>
                ...
                <div style={{ background: 'rgba(0,0,0,0.3)', padding: '1rem', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontWeight: 600, color: 'var(--accent-purple)', marginBottom: '0.25rem' }}>Business Impact (BI)</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>BI = 0.40*Financial + 0.25*Reversibility + 0.20*Sensitivity + 0.15*External. BI &ge; 75 &rarr; L3 Floor.</div>
                </div>
              </div>
            </div>

            {/* Last Evaluated Risk Inspector */}
            {lastGovernanceResult && (
              <div className="glass-card">
                <h3 style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '1rem' }}>Last Evaluation Risk Breakdown</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: '0.25rem' }}>
                      <span>Business Impact Score (BI)</span>
                      <span style={{ fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{lastGovernanceResult.risk_assessment.business_impact} / 100</span>
                    </div>
                    <div style={{ height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ width: `${lastGovernanceResult.risk_assessment.business_impact}%`, height: '100%', background: 'var(--accent-purple)' }} />
                    </div>
                  </div>

                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginBottom: '0.25rem' }}>
                      <span>Composite Risk Score (CR)</span>
                      <span style={{ fontWeight: 700, fontFamily: 'var(--font-mono)' }}>{lastGovernanceResult.risk_assessment.composite_risk}</span>
                    </div>
                    <div style={{ height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ width: `${lastGovernanceResult.risk_assessment.composite_risk * 100}%`, height: '100%', background: lastGovernanceResult.risk_assessment.composite_risk > 0.4 ? 'var(--accent-rose)' : 'var(--accent-emerald)' }} />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div>
            <div className="glass-card" style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span className="badge badge-review" style={{ fontSize: '0.9rem' }}>
            ...
            {pendingReviews.length === 0 ? (
              <div className="glass-card" style={{ textAlign: 'center', padding: '3rem' }}>
                ...
                <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Trigger SCENARIO B in the Hero tab to send a ₹50L payment to this review queue.</p>
              </div>
            ) : (
                  ...
                  <div key={item.action_id} className="glass-card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                      <div>
                        ...
                        <h4 style={{ fontSize: '1.1rem', fontWeight: 600 }}>{item.action_type} &rarr; {item.target}</h4>
                      </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                      ...
                      <button
                        ...
                        onClick={() => handleReviewAction(item.action_id, 'APPROVE')}
                      ...
                      <button
                        className="btn btn-scenario-malicious"
                        onClick={() => handleReviewAction(item.action_id, 'REJECT')}
                        style={{ padding: '0.6rem 1.25rem' }}
                      >
                        REJECT ACTION
                      </button>
          ...
          <div>
            <div className="glass-card" style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button className="btn btn-scenario-malicious" onClick={handleTamperSimulation} style={{ fontSize: '0.85rem' }}>
                  <AlertTriangle size={14} /> Simulate Unauthorized DB Tampering
                </button>
              </div>
